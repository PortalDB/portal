...
object Driver {
  def main(args: Array[String]) = {
    ...
    for (i <- 0 until args.length) {
      //aggregate operation
      if (args(i) == "--agg") {
        var sem = AggregateSemantics.Any
        var runWidth = 1; 
        ...
        if (changedType) {
          ...
          result2 = result2.aggregate(resolution, sem, sem, aggFunc2, aggFunc2)
        } else {
          if (partAgg) {
            runWidth = result.resolution.getNumParts(resolution, LocalDate.now())
            result = result.partitionBy(partitionType, runWidth, numParts)
          }
          result = result.aggregate(resolution, sem, sem, vAggFunc, eAggFunc)
        }
