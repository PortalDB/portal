...
class LocalQueries(dataset: String) {
  ...
  def getNodeHistory(id: VertexId, intv: Interval): RDD[(VertexId, (Interval,Any))] = {
    ...
    if (dfs.schema.fields.size > 3)
      dfs.filter("vid == " + id).filter("NOT (estart >= " + secs1 + " OR eend <= " + secs2 + ")").rdd.map(r => (r.getLong(0), (Interval(r.getLong(1), r.getLong(2)), r.get(3))))
    else if (dfs.schema.fields.size > 2)
      dfs.filter("vid == " + id).filter("NOT (estart >= " + secs1 + " OR eend <= " + secs2 + ")").rdd.map(r => (r.getLong(0), (Interval(r.getLong(1), r.getLong(2)), null)))
    else
  ...
  def getEdgeHistory(srcId: VertexId, dstId: VertexId, intv: Interval): RDD[((VertexId, VertexId), (Interval, Any))] = {
    ...
    if (dfs.schema.fields.size > 4)
      dfs.filter("vid1 == " + srcId + " and vid2 == " + dstId).filter("NOT (estart >= " + secs1 + " OR eend <= " + secs2 + ")").rdd.map(r => ((r.getLong(0), r.getLong(1)), (Interval(r.getLong(2), r.getLong(3)), r.get(4))))
    else if (dfs.schema.fields.size > 3)
     dfs.filter("vid1 == " + srcId + " and vid2 == " + dstId).filter("NOT (estart >= " + secs1 + " OR eend <= " + secs2 + ")").rdd.map(r => ((r.getLong(0), r.getLong(1)), (Interval(r.getLong(2), r.getLong(3)), null)))
    else
