...
object TemporalAlgebra {
  ...
  def aggregate[T: ClassTag, K: ClassTag, V: ClassTag](rel: RDD[(T, Interval)], groupBy: T => K, seqFunc: (T, V) => V, aggFunc: (V, V) => V, zeroVal: V): RDD[((K, Interval), V)] = {
    val intervals: RDD[(K, List[Interval])] = rel.map(x => (groupBy(x._1), x._2)).aggregateByKey(List[Interval]())(seqOp = (u: List[Interval], v: Interval) => TemporalAlgebra.combOp(u, List[Interval](v)), TemporalAlgebra.combOp)
  ...
  def union[K: ClassTag, V: ClassTag](one: RDD[(K, (Interval, V))], two: RDD[(K, (Interval, V))]): RDD[((K, Interval), V)] = {
    ...
    val intervals: RDD[(K, List[Interval])] = un.aggregateByKey(List[Interval]())(seqOp = (u: List[Interval], v: (Interval, V)) => TemporalAlgebra.combOp(u, List[Interval](v._1)), TemporalAlgebra.combOp)
