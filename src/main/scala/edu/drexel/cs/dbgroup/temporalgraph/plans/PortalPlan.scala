...
abstract class PortalPlan extends QueryPlan[PortalPlan] {
   ...
   */
  final def execute(): TemporalGraphWithSchema[VertexEdgeAttribute,VertexEdgeAttribute] = {
    prepare()
    doExecute()
  }
   ...
   * Produces the result of the query as an TemporalGraphWithSchema[VertexEdgeAttribute,VertexEdgeAttribute]
   */
  protected def doExecute(): TemporalGraphWithSchema[VertexEdgeAttribute,VertexEdgeAttribute]
