...
import edu.drexel.cs.dbgroup.portal.portal._
...
class PortalContext(@transient val sparkContext: SparkContext) {
  ...
  protected[portal] lazy val analyzer: Analyzer =
    new PortalAnalyzer(sessionCatalog, functionRegistry, conf)

  protected[portal] lazy val optimizer: Optimizer = new PortalOptimizer(sessionCatalog, conf)

  protected[portal] lazy val planner = new PortalPlanner()
  ...
  protected[portal] class QueryExecution(val logical: LogicalPlan) {
