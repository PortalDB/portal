...
\reminder{I don't understand how this is true.  Let's think this
  through precisely for all operations and write down a proof sketch.
  If we are not sure about this, or don't have time to finish the
  proof, I prefer to remove the lazy coalescing passage and leave this
  for the query optimization paper.  If removed - remember to remove
  the forward pointer at the end of 3.1.}

\eat{For example, \insql{slice} operation is agnostic to whether a TGraph
is coalesced and does not change the coalesce state.  \insql{select}
operation, on the other hand, is only correct if the data is coalesced
when the selection predicate references the time period.  We analyzed
all operations for their coalescing requirements and whether the
operation causes uncoalescing (Table~\ref{tab:coalesce}).}
