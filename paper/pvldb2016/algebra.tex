...
{\bf Projection may uncoalesce \tav and \tae in \tve.  Projection may
  uncoalesce \trg.} Consider \insql{T1} in Figure~\ref{fig:ve}, and
...
analyze an evolving graph at coarser levels of granularity,
aggregating multiple consecutive representative graphs into a single
representative graph.  Our approach is based on stream aggregation
work by Li, et al.~\cite{Li2005}, with the addition of quantifiers. 
...
into 1.  This is similar to slide-by-row window specification in
stream aggregation~\cite{Li2005}.  
...
most | at least $n$ \}, where $n$ is a decimal representing a ratio.
      ...
      a large portion of the window.  Our aggregation quantifiers are
      inspired by generalized quantifiers of~\cite{Hsu1995} with
      n-place delimiters.

Consider for example 

\eat{ $Q(R)$ as a Boolean-valued function of a
  relation''~\cite{Hsu1995}.  A quantifier contains an n-place
  determiner, e.g., ``at least one vertex in each window for each
  group'' is a 2-place determiner quantifier.  \tg algebra supports
  determiners from the set $\{at\ least\ one, all, most,
  at\ least\ n\}$, where $n$ is an integer representing a ratio.
  $all$ is a usual universal quantifier that in standard SQL can be
  achieved with the use of two \insql{NOT EXISTS}.}
