...
graph that evolves continuously over time.\eat{ Our choice to
  represent continuous graph evolution is in contrast with a
  less-general discrete snapshot-based representation. } 
...
Figure~\ref{fig:tg_ve}.\eat{ In its basic form, a \tg is represented
  by a pair of temporal SQL relations $V$ and $E$ that capture
  evolution of graph topology.  A \tg may optionally represent
  evolution of vertex and edge attributes with relations $A^{V}$ and
  $A^{E}$.}

%$A^{V}_1, \ldots, A^{V}_n$ and $A^{E}_1, \ldots, A^{E}_m$
...
\insql{T} optionally includes vertex attribute relation $A^{V}$ and an
...
%{\bf More about vertex and edge attributes.} 
...
for each $[t_1, t_2)$ and $[t_2, t_5)$.  This would in turn require
    ...
    our representation.\eat{  We will discuss how integrity of the model is
