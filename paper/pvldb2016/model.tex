...
vertex attributes \avv, and a set of edge attributes \aee.
...
\vera{GraphX does not use the property graph model. It uses generics
  and says that at compile time the type of attribute has to be known,
  but otherwise it can be anything.}
...
\vera{It may be worth noting that we are using what's called a
  timestamp tuples approach rather than timestamp attributes in our
  logical model. }
...
for each $[2/15, 5/15)$ and $[5/15, 10/15)$.  This would in turn
    ...
    our representation.\eat{ We will discuss how integrity of the
      ...
      Sections~\ref{sec:algebra} and~\ref{sec:sys}.}  Another reason
    for optional \tav and \tae is that in many cases we are interested
    in graph topology only, and in that case \tv and \te are
    sufficient.
