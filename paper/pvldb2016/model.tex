...
\begin{definition}[Period sequence]
...
Armed with Definitions~\ref{def:period} and~\ref{def:seq}, we now
...
\eat{The time periods of all graph entities, i.e. vertices and edges,
  are maximally coalesced.  That means that all the tuples in $V$ and
  $E$ are distinct in all attributes, including the time periods, and
  that time periods do not overlap for any individual entity and are
  maximal.} 
...
An example of a \tg is given in Figure~\ref{fig:tg}, with 
