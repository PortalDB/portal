...
\ql \eat{, available in several interesting variants, }can be used to find a
...
variants of {\em temporal union and intersection} that enable this
kind of analysis.
...
\item We propose a flexible representation of evolving graphs, called
  a \tg, which captures evolution of graph topology, and of attributes
  of vertices and edges.  Conceptually, a \tg corresponds to a
  collection of coalesced~\cite{DBLP:conf/vldb/BohlenSS96} temporal
  SQL relations~\cite{DBLP:journals/sigmod/KulkarniM12}, with
  appropriate integrity constraints.  Practically, to support
  distributed computation and evolution of vertex and edge schemas, we
  represent \tgs as a collection of RDDs \reminder {more details
    here.}  The \tg model is presented in Section~\ref{sec:model}.

\eat{We build on the temporal features of the SQL:2011
  standard~\cite{DBLP:journals/sigmod/KulkarniM12}, and encode the
  toploogy of an evolving graph as a pair of coalesced temporal SQL
  relations, with appropriate integrity constraits.  Vertex and edge
  attributes are likewise encoded in coalesced temporal SQL relations.
}

\eat{ Importantly, a \tg is
  coalesced~\cite{DBLP:conf/vldb/BohlenSS96}, that is, each vertex
  (resp. edge) of a \tg is represented exactly once for each time
  period of maximal length when the vertex (resp. edge) is present,
  and when none of the values of its attributes change.}

\item 

\end{itemize}  We define a \tg algebra, which includes a variety of operators
such as temporal and structural selection, temporal and structural
aggregation (with novel quantification mechanisms)

In this paper we develop \ql, a system for
efficient querying and exploratory analysis of evolving graphs.  

\ql
operates over \tgs, which represent graphs that evolve over time, and
implement
