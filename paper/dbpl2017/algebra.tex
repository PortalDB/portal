...
with a focus on analysis over time\eat{, and motivated by our examples and
those we found in the literature}.
...
\begin{definition}[Identity-equivalent]
...
with the \tg model.  The \insql{resolve} primitive is used in some of
...
\tg history.  The \insql{trim} operator computes a new \tg, limited
...
nested attribute \insql{a}.  Because node periods can be accessed but
...
automatically by the model.  The \insql{edge-map} is defined
similarly.
...
To state this formally, we first modify the definition of a
...
\begin{itemize}[noitemsep,topsep=3pt,itemindent=\dimexpr\labelwidth+\labelsep\relax,leftmargin=5pt]
\item $\Pi' = \Pi \cup M$,
...
relational aggregation functions \insql{sum}, \insql{min},
\insql{max}, and \insql{count} are supported.  Additionally, set-based
...
otherwise the same as the nontemporal one.  Intuitively\eat{, like in
  the non-temporal version,} the nodes are computed by applying the
...
\centering
\setlength\tabcolsep{1.8pt}
\caption{Attribute-based node creation on property \insql{school}.}
\vspace{-0.2cm}
\label{tab:nodecra1}
%\resizebox{\columnwidth}{!}{%
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}|c|c|c|p{1.84in}|c|}
...
\begin{example}
\vspace{-0.2cm}
Consider our running example \tg \ttt\eat{ (Table~\ref{tab:tg})}.  We can
...
\subsection{Set-based operators}  
...
done in any order\eat{ or in parallel}.
...
representing journeys.  \eat{Recall that a journey is a path in the
