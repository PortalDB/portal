...
\underline{vid2}:int).  Consider the \insql{SQ} query below, which
shows \insql{vid} and \insql{tr} values of 20 vertices with the most
significantly increasing \insql{pagerank} trend.  
...
The important part of $Q12$ is the use of \insql{T5.toVerticesFlat()}
in the \insql{From} clause.  This is a multi-step operation provided
by the \ql framework, which starts by collecting all vertices in the
union of snapshots of \insql{T5} into a single nested vertex
collection, and associating \underline{vid}:int with a time-indexed
map of vertex attributes.  Next, the nested collection is flattened
into \insql{VF} with the schema VF (\underline{vid}:int,
\underline{start}:date, \underline{end}:date, tr:float, mx:float).
\insql{VF} can be used in \insql{SQL} queries.  The \ql framework also
provides an operation that returns a flattened collection of edges,
called \insql{toEdgesFlat()}.

\eat{
An \insql{Output} statement of this kind is used to inspect results,
and is similar to \insql{Describe} in SQL.  It does not produce a \tg
as output, and so cannot be considered a query, since its result
cannot be composed with \ql operators.}

\eat{The \insql{Output} statement allows the user to inspect vertices or
...
optional \insql{Order by} clause can be used to sort results on one or
...
methods are \insql{vertices()}, \insql{verticesFlat()},
\insql{edges()}, and \insql{edgesFlat()}.  \insql{Output} produces
collections computed by \insql{vertices()} or
\insql{edges()}. }
