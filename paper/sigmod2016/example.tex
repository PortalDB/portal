...
$Q1$ performs temporal selection --- its result is a \tg that
...
match, or be coarser than, the time unit of \insql{T1}. \reminder{What
  ...
  the boundary?}\vera{Response: it dilates, meaning the resulting selection will start with the interval that contains the specified start date and end with the interval that contains the specified end date.}
...
API. \reminder{Is what I'm saying true?  What other analytics do we
  support? How difficult is it to provide an API to support custom
  local / Pregel-style analytics?}\vera{Yes, our api supports degrees,
  connected components, shortest paths, pagerank, and defining any
  pregel operation. The remaining question is how to tie that into the
  language which is not api-based.}
...
{\bf Loading filesystem data.  Referring to query results.}

Queries $Q1$ through $Q8$ referred to \tg variables in the
\insql{From} clause.  The value of a \tg variable is assigned by the
\insql{Into} clause.  This value may be computed by any \ql query
...
\insql{TSelect} clause must specify the structural schema.  On the
other hand, when a \tg variable takes on a value computed by a query,
as is the case with \insql{T4} in query $Q10$, its structural schema
is determined by the query result.  That is, the structural schema of
\insql{T4} is the same as that of \insql{T1}.
