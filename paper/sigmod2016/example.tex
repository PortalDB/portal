...
operations available: \insql{any}, \insql{first}, \insql{last},
...
structural aggregation is due to temporal intersection (\insql{TAnd})
or union (\insql{TOr}), but only as part of a query that does temporal
aggregation (as in \insql{TGroup}).
...
$Q9$ is assigned to \insql{T5}.  \ql supports vertex and edge
transformation methods that expose the contents of a \tg as vertex and
edge collections.  These methods are \insql{vertices()},
\insql{verticesflat()}, \insql{edges()}, and \insql{edgesFlat()}.
...
\reminder{I'm not happy with this syntax, will work on this more.}
