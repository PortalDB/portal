...
to \insql{T5}, with the structural schema V(\underline{vid}:int,
tr:float, mx:float) ; E(\underline{vid1}:int, \underline{vid2}:int).
Consider the statement below, which shows \insql{vid} and \insql{tr}
...
The \insql{Output} statement allows the user to inspect vertices or
...
\insql{edges()}. \reminder{Return types for each.  Explain why we
  don't need to flatten here.}
