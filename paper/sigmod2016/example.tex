...
a consecutive subset of the snapshots of \insql{T1}, namely, $[2010,
  2011)$ through $[2013, 2014)$, and has the same structural schema as
    \insql{T1}.  \insql{Start} $t_1$ \insql{End} $t_2$ specifies a
    closed-open period $[t_1, t_2)$.  Its time unit must match, or be
      coarser than, the time unit of \insql{T1}.
...
\eat{ \reminder{What do we do if start or end falls within a snapshot,
    i.e., is not at the boundary?}\vera{Response: it dilates, meaning
    the resulting selection will start with the interval that contains
    the specified start date and end with the interval that contains
...
order of \insql{TAnd}, \insql{TGroup} and temporal selection is
...
\reminder{Discuss what happens when Start or End falls inside a
  ...
  interchangeable.}\vera{It is interchangeable in this example but is not in general. For example, if graph is in [2010,2016) and you want to agg by 2 and select 2013 to 2015, if agg is done first, the final result is 2 snapshots, but if select is done first, then only 1.}
...
referred to \tg variables in the \insql{From} clause.  The value of a
\tg variable is assigned by the \insql{Into} clause.  This value may
...
system, as in $Q9$, the \insql{TSelect} clause must specify the
...
$Q10$, the structural schema of the view \insql{T4} is the same as
that of \insql{T1}.
