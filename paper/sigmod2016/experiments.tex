...
data size, we ran a simple \insql{TSelect} query on nGrams:
...
\eat{ Because the load/materialize time varies so significantly
  ...
  reported with a warm start, with application of a desired partition
  strategy included during the loading process.}
...
data structure, it outperforms MGC and SG for the \insql{TGroup}
...
for the \insql{TGroup} query with \insql{All} semantics --- see
Appendix.
...
size and vary the aggregation width.\eat{, as can be seen in
