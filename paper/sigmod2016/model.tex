...
evolves over time.  We start by formally defining snapshots of an
evolving graph in Section~\ref{sec:model:structure}.  Then in
Section~\ref{sec:model:time} we show how graph evolution is
represented by assigning temporal meaning to sequences of snapshots.
...
$E$ are homogeneous sets. $G$ may represent a directed or an
undirected graph.  For undirected graphs we choose a canonical
representation of an edge, with $vid_1 \leq vid_2$ (self-loops are
allowed).

\begin{definition} [Structural union-compatibility]
Snapshots $G_1 = (V_1, E_1)$ and $G_2 = (V_2, E_2)$ are
union-compatible if $V_1$ and $V_2$ are union-compatible, and $E_1$
and $E_2$ are union-compatible.
\label{def:scompat}
\end{definition}

This is the standard union compatibility definition, which requires
that vertex schema $V$ and edge schema $E$ be the same for $G_1$ and
$G_2$.
...
\begin{definition} [Structural aggregation]  Consider a collection 
of $n$ union-compatible snapshots $\cal{G} = \{G_1 \ldots G_n\}$.
$\gamma = (\gamma^{V}, \gamma^{E})$ maps $\cal{G}$ to a snapshot $G$.
$\gamma^{V}$ and $\gamma^{E}$ specify aggregation behavior for the
vertices and for the edges, respectively.  For each, we define two
variants.  $\gamma^{V} = $ 
\label{def:sgroup}
\end{definition}
...
\begin{definition} [Temporal Aggregation]
...
in our formalism, or \tgs for short.  We conclude this section by
defining \tgs and \tg operations.
...
\label{def:tuc} 
$T_1$ and $T_2$ are union-compatible \tgs if they are both
