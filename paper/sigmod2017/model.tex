...
%
\eat{Each $g = (V_g, E_g, \avv_g, \aee_g)$, with $V_g \subseteq V$, $E_g
...
consecutive periods $p_1=[1/15, 2/15), \ldots, p_5=[7/15, 10/15)$.} \eat{ It
...
We use valid-time temporal SQL relations~\cite{DBLP:conf/vldb/BohlenSS96},
...
{\em \ve \tg}.  An example \eat{of a \ve \tg }is given in
Figure~\ref{fig:tg_ve}.  

\eat{
We will refer to the \rgs
(Definition~\ref{def:tg_abstract}) and \ve (Definition~\ref{def:tg})
representations of a \tg jointly as \insql{T}, and will disambiguate
...
added at time 5/15.  It is not required that a vertex or an edge be
represented in \tav and \tae --- a vertex or edge that never had any
associated properties will have no corresponding tuples in \tav or
\tae.

The statement that \tg is temporally coalesced means that each fact
(unchanging state of a vertex or edge) is represented exactly once for
...
time periods for a vertex or edge, i.e., periods $p$ and $q$ for which
...
The relations of \tve can be implemented in temporal SQL, as per the
