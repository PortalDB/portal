...
the bag of vertex or edge properties.\eat{, with the short-hand syntax
  $\map_{M_V:distinct,M_E:distinct} (\insql{T1})$.}  It may also be
...
Temporal aggregation is denoted $_{G_V}\gamma_{W,Q_V,Q_E,A_V,A_E}
(\ttt)$,\\ where $G_V$ are the grouping attributes, $W$ is the window
specification, $Q_V$ and $Q_E$ are vertex and edge aggregation
quantifiers, and $A_V$ and $A_E$ are the optional aggregation
functions.  It produces a consolidated evolving graph with specific
...
aggregation window is one change, then the operation devolves into
pure structural reduce or node creation, as classified by
Wood~\cite{Wood2012}.  If the grouping attribute is the vertex $vid$,
then the aggregation is purely temporal, with no structural aspect.
...
values for vertex and edge properties \eat{that are }representative of
...
\insql{sum} | \insql{average} \}, which have their customary meaning.
We also support \{ \insql{any} | \insql{first} | \insql{last} |
\insql{trend} | \insql{list} \}, which are possible to compute because
properties being aggregated have temporal information.  \insql{first}
and \insql{last} refer to earliest/latest non-null value of the given
property in the window, while \insql{trend} computes the slope of the
least squares line using linear regression, making an adjustment when
a value is missing, and \insql{list} associates with a key an ordered
...
1.  Next, as part of $_{G_V}\gamma_{P,Q_V}(\tv)$ computation, we assign
each tuple from \tv to an aggregation period (this requires
...
processed similarly by $_{G_V}\gamma_{P,Q_E}(\te)$ on line 3.  Note that
...
compute $_{G_V}\gamma_{P,A_V}(\tav)$ and $_{G_V}\gamma_{P,A_E}(\tae)$,
...
period, and applying the aggregate functions over each group. \eat{ It
  ...
  in the result of $\gamma_{P}$ may belong to multiple periods.}

Temporal aggregation over \trg is computed by first calculating time
periods from $W$ and \trg, and then reducing and combining the
representative graphs directly.
...
\STATE  $\tv' = \cl (_{G_V}\gamma_{P,Q_V}(\tv))$\\
\STATE  $\te' = \cl (_{G_V}\gamma_{P,Q_E}(\te))$\\
\STATE  $\tav' = \cl (_{G_V}\gamma_{P,A_V}(\tav))$\\
\STATE  $\tae' = \cl (_{G_V}\gamma_{P,A_E}(\tae))$\\
...
structural aggregation only ($W=1~\textsf{change}$), and
