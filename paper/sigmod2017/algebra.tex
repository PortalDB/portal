...
and by non-temporal attributes (e.g., vertex properties).  \vera{This
  section has to be revised to explain how structural aggregation is
  supported.}

\eat{
Also unlike~\cite{Li2005}, we do not currently support aggregation
...
applying the aggregate functions over each group. \eat{ It is possible
  but inconvenient to express the operations on lines 2---5 in
...
($W=3~\textsf{changes}$).  Both are applied to \insql{T1} in our
