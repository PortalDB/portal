...
translation is sound.  Recall from Definition~\ref{def:tg} that a
valid \tg must satisfy four requirements: {\bf R1}: Unique vertices
and edges, {\bf R2}: Unique attribute values, {\bf R3}: Referential
integrity, and {\bf R4}: Coalesced.  We now describe three primitives
that will be used together with temporal relational algebra operations
to ensure soundness of \tga.
...
application of operations that uncoalesce.  Note that the coalesce
primitive can be implemented in standard relational algebra~\cite{},
of which TRA is a superset.  Note also that if a DBMS supports
automatic coalescing, this primitive is not necessary.  Finally, eager
...
computing over uncoalesced inputs.  We will revisit this point in
...
computes a temporal group-by of an attribute relation $\mathbf{r}$ by
...
name $\mathbf{k_i}$, and aggregates values within each graoup using
...
\eat{The reduce function may, for example, pick the left, the right,
  or an arbitrary element, compute the value of an aggreagte function
...
subgraph if \tve, with vertices defined by the temporal conjunctive
query (TCQ) $q^t_v$.  Note that this is a subgraph query, and so no
new vertices are created, i.e., $\tv' \subseteq^T \tv$.  %As long as
%$q$ is a valid TCQ that returns a subset of vertex ids from \tv, the
%output is guaranteed to be a valid \tg.
...
Queries $q^t_v$ and $q^t_e$ may use any of the consitutent relations
of \tve to compute the vertex (resp. edge) sets.  Further, since these
are TRA queries, they may explicitly reference temporal information,
and so require that all temporal relations over which they operate
must be coalesced~\cite{DBLP:reference/db/Bohlen09}.

\subv{q^t_v}{\ttt} must invoke $\coal{\tv'}$ to enforce {\bf R1},
     {\bf R4}, and $\constr{\te'}{\tv'}$, $\constr{\tav'}{\tv'}$,
     $\constr{\tae'}{\te'}$ to enforce {\bf R3}.
%
\subv{q^t_e}{\ttt} must invoke $\coal{\te'}$ to enforce {\bf R1}, {\bf
  R4}), and $\constr{\tae'}{\te'}$ to enforce {\bf R3}.  %No further
...
\paragraph*{Map}  Temporal vertex-map and edge-map operations apply
...
To produce a valid \tg, $\vmap{\tve}$ must invoke $\coal{\tv'}$ and
$\emap{\tve}$ must invoke $\coal{\te'}$, enforcing {\bf R2}.
...
past year.  Aggregation is denoted $\agg{cond,msg,f}{\ttt}$, where
...
triplet and produces a $(v,p,m)$ message $m$ of the form $key=val$,
sent to vertex $v$ at triplet time $p$, and $f$ is an aggregation
function.  The result is a new isomorphic graph $\ttt'$:
$\agg{cond,msg,red}{\ttt} = \{ \tv, \te, \tav', \tae \}$, where $\tav'
= \pi^T_{v,a_1+a_2}(\tav \bowtie^T_v$ $_v\vartheta^T_{f} (msg
(\sigma_{cond} (\tae \bowtie^T_{v_1} \tav \bowtie^T_{v_2}
\tav))))$.  The temporal join is used to add the new properties to the
output since it may have different periods of validity.  For example,
while each vertex in \tg may remain unchanged for the whole duration,
