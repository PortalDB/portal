...
\julia{Vera will revise this section to spell out the details of how
...
non-empty intersection with $\bc$.  To evaluate $\tau_{\bc} (\tve)$,
we apply $\tau_{\bc}$ to each of the four constituent relations of
\tve: $\tau_{\bc} (\tv) = \{ (v, \bp \cap \bc)~~|~~(v, \bp) \in \tv
...
\eat{ Slice does not uncoalesce and does not require FK enforcement.
  The proofs for this and similar statements below are included in }
\julia{Soundness info goes here: what needs to be checked and
  enforced.}  Proofs are included in Appendix~\ref{sec:app1}.
