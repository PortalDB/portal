...
first execute $R = g_1,\ldots,g_i \gamma^T (\tv \rightouterjoin^T_v
...
\sigma^T_{skolem(g_1,\ldots,g_i)}R$.  Vertex attributes in $\tav'$ are
computed as follows: $\tav' = \sigma^T_{skolem(g_1,\ldots,g_i), a}
(\resolve{f_1(k_1), \ldots, f_n(k_n)}{R})$.  Note the use of the
resolve primitive to reconcile attribute values within a group.
...
conjunctive query over $\tve$ that returns the set of edges induced by
the partitioning, and then invoke edge creation. \julia{describe how
  edges are created}

\paragraph*{Window-based node creation} This operation is denoted
$\insql{node}^T_w(w,q_v,q_e,f_1(k_1),\ldots,f_n(k_n),\tve)$, where $w$
is the window specification, $q_v$ and $q_e$ are vertex and edge
quantifiers, and each $f_i(k_i)$ specifies an aggregation function
$f_i$ to be applied to a property name $k_i$.  This operation
...
changes that occured in \tve (affecting any of its constituent
...
observing different kinds of temporal evolution.  For example, to
observe only strong connections over a volatile evolving graph, we may
want to only include vertices that span the entire window
...
Specifically, we compute $L(v_1,v_2,a,p) = q(\ttt_1, \ttt_2)$, and
then set $\te' = \coal{\pi_{v1,v2,p} L}$, and $\tae' =
...
operator by a combinatin of node creation
(Section~\ref{sec:algebra:ncreate} and edge creation.
