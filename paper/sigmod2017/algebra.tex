...
view $L(v_1,v_2,v_1.a,v_2,e.a,p)$, where $v_1$ refers to the vertex
...
associated time period.  $L$ is computed in an obvious way with a
temporal join, with $\tav$ and $\tae$ being outer-joined, since a
vertex / edge is not required to specify an attribute.  When \tve is
directed, and direction of the edge is important, it can be accounted
for in the way the join is set up (e.g., mapping $v_2$ in \tae to
$v_1$ in $L$ if the goal is to aggregate information on incoming
edges).

Aggregation is denoted $\agg{dir,cond,f_m,f_a}{\ttt}$, where $dir$ is
the direction of the edge (one of 'right', 'left' or 'both') that
determines how $L$ is computed, $cond$ is a temporal predicate over
$L$, $f_m$ is a map function that emits a value for each tuple in the
result of $\sigma^T_{cond}(L)$ (e.g., 1 for computung degree of $v_1$
or $v_2.a.get(country)$ for computing the set of countries in which
...
As the final step, $\tav' = \pi_{v, \tav.a \cup R.a} (\tav
