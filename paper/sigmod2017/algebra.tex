...
\item [R2: Coalesced] Value-equivalent tuples in all relations of \tve
...
\julia{Referential integrity is always by key.  There is not need to
...
$\mathbf{r}$ with respect to relation $\mathbf{s}$.  For example, this
primitive is used to remove edges from \te for which one or both
vertices are absent from \tv, and to trim edge validity period to be
within the validity periods of the associated vertices. \julia{Does
  this latter case arise?}
...
time point $c$ accept a user-specified reduce function $\mathbf{red}$.

\julia{No need for a definition.  Do we need period to be part of the
  resolve function?  If so, what is the default behavior?  And how
  does one compute, e.g., a union of the periods if they are not
  adjacent?  Generally, it's not clean to allow explicit manipulation
  of timestamps in a temporal model - it's OK to refer to them in the
  where clause, but not to compute them, because that changes model
  expressiveness drastically --- you can specify an arbitrary function
  that ``creates time''.}
