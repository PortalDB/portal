...
To compute $\tv'$, we apply the split primitive to \tv, group by vid,
select only those vertices which meet the quantification, and finally
coalesce: $\tv' = \coal{\sigma^T_{q_V} (v \gamma^T_{\cup
    p}(\wsplit{s}{w}{\tv}))}$.  Similarly for $\te'$.  To compute
attribute relations we split, resolve with the aggregation functions,
and enforce FK constraints: $\tav' =
